...
//exclusion
    ...
    //inclusion and exclusion only used at when we dont need _id
    db.movies.find({}, {_id: 0, name:1})

    //inclusion + filter
    db.movies.find({rating: {$gt: 8}}, {name:1, rating:1})
...
db.orders.insertMany(
    ...
    //using aggregate function with status urgent
db.orders.aggregate([ { $match: {status: "urgent"}}])

// calculat sum
db.orders.aggregate([ { $match: {status: "urgent"} }, {$group: {_id: "$productName" , totalquantity: {$sum: "$quantity"} } }])
